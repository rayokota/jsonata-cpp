...
jsonata::ordered_map<std::string, Functions::FunctionEntry>
                 ...
                 [](const Utils::JList& args) -> std::any {
                     ...
                     std::string picture =
                         (args.size() >= 2 && isString(args[1]))
                             ? std::any_cast<std::string>(args[1])
                             : "";
                     jsonata::ordered_map<std::string, std::any> options;
                     if (args.size() >= 3) {
                         try {
    ...
    try {
        ...
        // Check for numeric types
            ...
            const auto& map = std::any_cast<
                const jsonata::ordered_map<std::string, std::any>&>(arg);
            result = !map.empty();
        }
        // Functions are falsy
    ...
    try {
        ...
        if (isArray(arg)) {
            ...
            const auto& map = std::any_cast<
                const jsonata::ordered_map<std::string, std::any>&>(arg);
            for (const auto& entry : map) {
                jsonata::ordered_map<std::string, std::any> obj;
...
std::any Functions::sift(const Utils::JList& args) {
    ...
    try {
        ...
        if (objectArg.type() ==
            typeid(jsonata::ordered_map<std::string, std::any>)) {
            const auto& map = std::any_cast<
                const jsonata::ordered_map<std::string, std::any>&>(objectArg);
            for (const auto& entry : map) {
                // Prepare arguments for the predicate function
...
std::any Functions::each(const std::any& obj, const std::any& func) {
    ...
    try {
        ...
        if (obj.type() ==
            typeid(jsonata::ordered_map<std::string, std::any>)) {
            const auto& map = std::any_cast<
                const jsonata::ordered_map<std::string, std::any>&>(obj);
...
std::any Functions::lookup(const std::any& input, const std::string& key) {
    ...
    try {
        if (isArray(input)) {
            ...
            // detection
            const auto& map = std::any_cast<
                const jsonata::ordered_map<std::string, std::any>&>(input);
...
Utils::JList Functions::evaluateMatcher(const std::regex& pattern,
    ...
    for (; iter != end; ++iter) {
        ...
        jsonata::ordered_map<std::string, std::any> match;
...
jsonata::ordered_map<std::string, std::any> Functions::toJsonataMatch(
